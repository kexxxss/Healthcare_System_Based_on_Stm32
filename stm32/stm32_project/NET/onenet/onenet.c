...
const char *Topic = "/xm/addMember"; //mqtt主题，添加成员
//与设备mqtt通信宏
		...
		case MQTT_PKT_PUBLISH:														//接收的Publish消息
			...
			if(result == 0)
			{
                ...
                if(!json){
                    ...
                    json_value = cJSON_GetObjectItem(json,"addmember");//获取键为"addmember"的值
                    if(json_value->valueint == deviceid){                                   
                        json_value = cJSON_GetObjectItem(json,"memberfid");//获取指纹id
                        fid = json_value->valueint;
                        UsartPrintf(USART_DEBUG, "addquest,fid=%d\n",fid);
                        if(Add_FR(fid)){
                            UsartPrintf(USART_DEBUG, "指纹录入成功");
                            /* mqtt 报文发送,反馈录入结果*/
                            ...
                            UsartPrintf(USART_DEBUG, "指纹录入失败");
                            UsartPrintf(USART_DEBUG, "OneNet_Publish\r\n");
                            sprintf(
                                    buf,
                                    "{\"device\":%d,\"protocol\":\"%s\"}",
                ...
                cJSON_Delete(json); //删掉此次用于解析的json包
