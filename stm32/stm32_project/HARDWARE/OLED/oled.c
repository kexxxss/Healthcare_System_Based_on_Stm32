...
//num:汉字对应的序号
//取模方式 列行式
void OLED_ShowChinese_16(u8 x,u8 y,u8 num,unsigned char Hzk[][16])
{
	u8 i,m,n=0,temp,chr1;
	u8 x0=x,y0=y;
	u8 size3=2;
...
//size:字体大小
//n,m:数组行列数
//STR：存储该汉字语句串的取模数组，列行式
void OLED_SHOWSTR(u8 x,u8 y,u8 size1,u8 n,u8 m,unsigned char STR[][16])
{
    u8 i,x0,y0;
    x0 = x;
    y0 = y;
    for(i=0;i<(n/2);i++)
    {
        OLED_ShowChinese_16(x0,y0,i,STR);
        x0 += 18;
    }
}
...
//显示待机界面
void OLED_Show_Await_Face(void)
{
    //OLED显示，使用16*16字体时，一行能显示8个字，共四行
    OLED_Clear();
    ...
    OLED_ShowChinese(32,32,14,16);//待  
    OLED_ShowChinese(48,32,15,16);//机
    OLED_ShowChinese(64,32,12,16);//模
    OLED_ShowChinese(80,32,13,16);//式 
    OLED_ShowChar(98,32,'.',16);
    OLED_ShowChar(102,32,'.',16);  
    OLED_ShowChar(106,32,'.',16); 
    OLED_ShowChinese(16,48,51,16);//设  
    OLED_ShowChinese(32,48,52,16);//备
    OLED_ShowChar(50,48,'i',16); 
    OLED_ShowChar(58,48,'d',16);
    OLED_ShowChar(70,48,':',16);
    OLED_ShowNum(80,48,deviceid,3,16);
    OLED_Refresh();
}
